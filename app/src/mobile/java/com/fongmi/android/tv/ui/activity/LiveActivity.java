...
public class LiveActivity extends PlaybackActivity implements CustomKeyDown.Listener, TrackDialog.Listener, Biometric.Callback, PassListener, ConfigListener, LiveListener, GroupAdapter.OnClickListener, ChannelAdapter.OnClickListener, EpgDataAdapter.OnClickListener, CastDialog.Listener, InfoDialog.Listener, LivePlaybackHost {
    ...
    private void setViewModel() {
        ...
        mViewModel.epgInfo().observe(this, info -> {
            mBinding.control.title.setText(info);
            setMetadata();
        });
        ...
        mViewModel.getChannel().observe(this, item -> {
            if (mGroup != null) mGroup.setPosition(mChannelAdapter.setSelected(item.group(mGroup)));
            mChannel = item;
            setArtwork();
            showInfo();
            hideUI();
    ...
    private void setWidth(Live live) {
        ViewHelper.setWidth(mBinding.group, ViewHelper.getWidth(live, ResUtil.dp2px(48), 14));
    }

    @Override
    public void setWidth(Group group) {
        ViewHelper.setWidth(mBinding.channel, ViewHelper.getWidth(group, ResUtil.dp2px(56), ResUtil.dp2px(60), 14));
    }

    private void setWidth(Epg epg) {
        ViewHelper.setWidth(mBinding.epgData, ViewHelper.getWidth(epg, ResUtil.dp2px(48), 14));
    }

    private void setWidth(View view, int width) {
        ViewHelper.setWidth(view, width);
    }
