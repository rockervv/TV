...
public class CustomKeyDownLive extends GestureDetector.SimpleOnGestureListener {
    ...
    public boolean hasEvent(KeyEvent event) {
        return KeyUtil.isEnterKey(event) || KeyUtil.isUpKey(event) || KeyUtil.isDownKey(event) || KeyUtil.isLeftKey(event) || KeyUtil.isRightKey(event) || KeyUtil.isDigitKey(event) || KeyUtil.isMenuKey(event) || KeyUtil.isMediaKey(event) || event.isLongPress();
    }

    public boolean onKeyDown(KeyEvent event) {
        if (listener.dispatch(true)) return check(event);
        return false;
    }

    private boolean check(KeyEvent event) {
        if (KeyUtil.isActionDown(event) && KeyUtil.isLeftKey(event)) {
            ...
            return true;
        } else if (KeyUtil.isActionUp(event) && (KeyUtil.isEnterKey(event) || KeyUtil.isMediaKey(event))) {
