...
public class CustomKeyDownVod extends GestureDetector.SimpleOnGestureListener {
    ...
    public boolean hasEvent(KeyEvent event) {
        return KeyUtil.isEnterKey(event) || KeyUtil.isUpKey(event) || KeyUtil.isDownKey(event) || KeyUtil.isLeftKey(event) || KeyUtil.isRightKey(event) || KeyUtil.isMediaKey(event);
    }
    ...
    private void check(KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN && KeyUtil.isLeftKey(event)) {
            ...
            if (!event.isLongPress()) listener.onKeyUp();
            ...
            changeSpeed = false;
        } else if (event.getAction() == KeyEvent.ACTION_UP && (KeyUtil.isEnterKey(event) || KeyUtil.isMediaKey(event))) {
