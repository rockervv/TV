...
public class OkHttpStreamClient extends AbstractStreamClient<OkHttpStreamClient.Configuration, Call> {
    ...
    protected Call createRequest(StreamRequestMessage requestMessage) {
        ...
        requestMessage.getHeaders().forEach((key, value) ->
                value.stream().filter(Objects::nonNull).forEach(v -> builder.addHeader(key, v)));
        ...
        return () -> {
            try (Response response = call.execute()) {
                ...
                response.headers().names().forEach(name ->
                        response.headers(name).forEach(value -> upnpHeaders.add(name, value)));
