...
public class BrowseTree {
    ...
    private static ImmutableList<MediaItem> getChildrenInternal(@NonNull String parentId) {
        if (parentId.equals(ROOT)) return ImmutableList.of(VOD_FOLDER, LIVE_FOLDER);
        if (parentId.equals(VOD)) return VodBrowse.getHistory();
        if (parentId.equals(LIVE)) return LiveBrowse.getGroups();
        if (parentId.startsWith(LiveBrowse.LIVE_GROUP)) return LiveBrowse.getChannels(parentId);
        return ImmutableList.of();
    }

    @Nullable
    public static MediaItem getItem(@NonNull String mediaId) {
        if (mediaId.equals(ROOT)) return ROOT_ITEM;
        if (mediaId.equals(VOD)) return VOD_FOLDER;
        if (mediaId.equals(LIVE)) return LIVE_FOLDER;
        if (mediaId.startsWith(LiveBrowse.LIVE_GROUP) || mediaId.startsWith(LiveBrowse.LIVE_CH)) return LiveBrowse.getItem(mediaId);
        return VodBrowse.getItem(mediaId);
    }
    ...
    private static void clearPending(@NonNull String prefix) {
        for (String key : browseResultMap.keySet()) if (key.startsWith(prefix)) browseResultMap.remove(key);
    }
