...
public class LiveSetting {
    ...
    public static int getScale() {
        return Math.min(Math.max(Prefers.getInt("scale_live", PlayerSetting.getScale()), PlayerSetting.MIN_SCALE), PlayerSetting.MAX_SCALE);
    }

    public static void putScale(int scale) {
        Prefers.put("scale_live", Math.min(Math.max(scale, PlayerSetting.MIN_SCALE), PlayerSetting.MAX_SCALE));
    }
}
