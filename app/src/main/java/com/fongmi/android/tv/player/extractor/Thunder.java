...
public class Thunder implements Source.Extractor {
    ...
    public boolean match(Uri uri) {
        return Arrays.asList("magnet", "ed2k").contains(uri.getScheme());
    }
    ...
    public static class Parser implements Callable<List<Episode>> {
        ...
        public List<Episode> call() {
            ...
            try {
                ...
                for (TorrentFileInfo info : XLTaskHelper.get().getTorrentInfo(taskId.getSaveFile()).getMedias()) {
                    items.add(create(info));
                }
                return items;
            } finally {
