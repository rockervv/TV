...
public class DLNAAvTransportImpl extends AbstractAVTransportService {
    ...
    private Map<String, String> parseHeaders(String metaData) {
        try {
            ...
            return content.getItems().stream()
                    .flatMap(item -> item.getProperties().stream())
                    .filter(p -> p instanceof DIDLObject.Property.DC.DESCRIPTION)
                    .findFirst()
                    .map(p -> App.gson().<Map<String, String>>fromJson(p.getValue().toString(), TypeToken.getParameterized(Map.class, String.class, String.class).getType()))
                    .orElse(new HashMap<>());
    ...
    protected TransportAction[] getCurrentTransportActions(UnsignedIntegerFourBytes instanceId) {
        switch (currentState) {
            case PLAYING:
                return withNext(TransportAction.Pause, TransportAction.Stop, TransportAction.Seek);
            case PAUSED_PLAYBACK:
                return withNext(TransportAction.Play, TransportAction.Stop, TransportAction.Seek);
            default:
                return withNext(TransportAction.Play);
        }
    }
    ...
    public void fireStateChange(RenderState state) {
        try {
            ...
            getLastChange().setEventedValue(getDefaultInstanceID(), new AVTransportVariable.TransportState(currentState));
