...
class LiveBrowse {
    ...
    static ImmutableList<MediaItem> getGroups() {
        ...
        for (Group group : liveHome().getGroups()) builder.add(BrowseTree.folder(LIVE_GROUP + group.getName(), group.getName()));
    ...
    static ImmutableList<MediaItem> getChannels(@NonNull String parentId) {
        ...
        Group group = null;
        for (Group item : liveHome().getGroups()) if (item.getName().equals(groupName)) { group = item; break; }
        if (group == null) {
            liveCountMap.put(groupName, 0);
            return ImmutableList.of();
        }
        for (String key : liveNavMap.keySet()) if (key.startsWith(groupName + '|')) liveNavMap.remove(key);
        for (String key : livePosMap.keySet()) if (livePosMap.get(key).groupName.equals(groupName)) livePosMap.remove(key);
        List<Channel> channels = group.getChannel();
        liveCountMap.put(groupName, channels.size());
        ImmutableList.Builder<MediaItem> builder = ImmutableList.builder();
        for (int i = 0; i < channels.size(); i++) builder.add(indexChannel(groupName, channels.get(i), i));
        return builder.build();
    }
    ...
    private static String ensureLoaded() {
        ...
        if (count == null || count == 0) return null;
        for (Map.Entry<String, LiveEntry> e : livePosMap.entrySet()) {
            if (e.getValue().groupName.equals(groupName) && e.getValue().channel.getName().equals(channelName)) return e.getKey();
        }
        return null;
    }
    ...
    private static class LiveEntry {
        ...
        String groupName;
        int index;
        LiveEntry(Channel channel, String groupName, int index) {
